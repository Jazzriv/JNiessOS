...
os.environ["DJANGO_SETTINGS_MODULE"] = "sass.tests.django_settings"
...
class LessTestCase(TestCase):
  ...
  border: 2px;
    ...
    def test_external_sass(self):

        template = Template("""
        {% load sass %}
        {% sass "styles/test.scss" %}
        """)
        compiled_filename_re = re.compile(r"SASS_CACHE/styles/test-[a-f0-9]{12}.css")
  ...
  background-image: url('/media/images/header.png');
        ...
        source_path = os.path.join(self.django_settings.MEDIA_ROOT, "styles/test.sass")
        ...
        time.sleep(self.django_settings.SASS_MTIME_DELAY)
